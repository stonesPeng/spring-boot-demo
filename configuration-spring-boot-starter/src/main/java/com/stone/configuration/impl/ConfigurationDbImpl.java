...
public class ConfigurationDbImpl extends AbstractConfiguration implements WritableConfiguration {
	...
	private static final Logger log = LoggerFactory.getLogger(ConfigurationDbImpl.class);
	...
	public ConfigurationDbImpl(DataSource dataSource, String tableName, String keyColumn, String valueColumn) {
		...
		Assert.notBlank(tableName, "Table name is blank!");
		Assert.notBlank(tableName, "Key column is blank!");
		Assert.notBlank(tableName, "Value column is blank!");
	...
	public void load() {
		hTable = dbUtils.load();
		log.debug("Configuration info loaded from database at {}", new Date());
	}
