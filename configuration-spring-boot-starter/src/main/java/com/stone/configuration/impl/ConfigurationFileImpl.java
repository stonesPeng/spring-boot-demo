...
public class ConfigurationFileImpl extends AbstractConfiguration implements WritableConfiguration {
	private static final Logger log = LoggerFactory.getLogger(ConfigurationFileImpl.class);
	...
	public void load() {
		...
		try {
			...
			log.debug("Load configuration from {} at {}", file.getAbsolutePath(), new Date());
		} catch (IOException e) {
	...
	private void store(Properties props, BufferedWriter out, String comments) throws IOException {
		...
		log.debug("Save configuration to {} at {}", file.getAbsolutePath(), new Date());
	}
